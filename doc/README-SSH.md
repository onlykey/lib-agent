...
See `onlykey-agent -h` for details on supported options and the configuration file format.

If you'd like a onlykey-style PIN entry program, follow [these instructions](README-PINENTRY.md).

## Usage

Use the `onlykey-agent` program to work with SSH. It has three main modes of operation:
...
Note the `--` separator, which is used to separate `onlykey-agent`'s arguments from the SSH command arguments.

Example:
```
 onlykey-agent -e ed25519 bob@example.com -- rsync up/ bob@example.com:/home/bob
```
...
##### 3. Connect to a server directly via `onlykey-agent`

If you just want to connect to a server this is the simplest way to do it:

```
$ onlykey-agent user@remotehost -c
```
...
Run:
	...
	/tmp $ onlykey-agent user@ssh.hostname.com -v > hostname.pub
	2015-09-02 15:03:18,929 INFO         getting "ssh://user@ssh.hostname.com" public key from onlykey...
	2015-09-02 15:03:23,342 INFO         disconnected from onlykey
...
For more details, see the following great blog post: https://calebhearth.com/sign-git-with-ssh
	...
	$ onlykey-agent -e ed25519 user@host --shell
	$ ssh-add -L
...
generated by onlykey. I.e. `/home/myUser/.ssh/onlykey.conf` with one public key per line.
This is a more convenient way to have a systemd setup that has to handle multiple
keys/hosts.

When updating the file, make sure to restart onlykey-agent.
...
systemctl --user enable onlykey-ssh-agent.socket
```

##### 3. Add this line to your `.bashrc` or equivalent file:

```bash
export SSH_AUTH_SOCK=$(systemctl show --user --property=Listen onlykey-ssh-agent.socket | grep -o "/run.*" | cut -d " " -f 1)
```

Make sure the SSH_AUTH_SOCK variable matches the location of the socket that onlykey-agent
is listening on: `ps -x | grep onlykey-agent`. In this setup onlykey-agent should start
...
```
$ onlykey-agent -e ed25519 git@github.com | tee ~/.ssh/onlykey-github.pub
ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIvcbhXyaXXNytCLTDfEMlLuwEhtfo0XmPP1U5RsnOZ4 <ssh://git@github.com|ed25519>
```
##### sign the given file using onlykey
```
$ onlykey-agent -e ed25519 git@github.com -- ssh-keygen -Y sign -f ~/.ssh/onlykey-github.pub -n file README.md
...
Note that your local SSH configuration may ignore `onlykey-agent`, if it has `IdentitiesOnly` option set to `yes`.
...
If you are failing to connect, save your public key using:
	...
	$ onlykey-agent -vv foobar@hostname.com > ~/.ssh/hostname.pub
...
Then, the following commands should successfully command to the remote host:
	...
	$ onlykey-agent -v foobar@hostname.com -s
	$ ssh foobar@hostname.com

or,

	$ onlykey-agent -v foobar@hostname.com -c
